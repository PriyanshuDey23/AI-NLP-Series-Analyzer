...
class JutsuClassifier():
    def __init__(self,
        ...
        if not huggingface_hub.repo_exists(self.model_path):
            ...
            # load_data method is called to load the data from the specified data_path
            train_data, test_data = self.load_data(self.data_path)
            train_data_df = train_data.to_pandas()
            test_data_df = test_data.to_pandas()


            # The training and testing data are concatenated into a single DataFrame all_data. 
            # The get_class_weights function is then called to compute the class weights for the data.
            all_data = pd.concat([train_data_df, test_data_df]).reset_index(drop=True)
            class_weights = get_class_weights(all_data) # Imported

            self.train_model(train_data, test_data, class_weights)

        self.model = self.load_model(self.model_path) # At last we load it after training
    ...
    def train_model(self, train_data,test_data,class_weights):
        ...
        data_collator = DataCollatorWithPadding(tokenizer=self.tokenizer) # Data collator that will dynamically pad the inputs received.

        training_args = TrainingArguments(
            output_dir = self.model_path,
            learning_rate=2e-4, # How big the steps are
            per_device_train_batch_size=8, # Train batch
        ...
        trainer.set_class_weights(class_weights) # CLass weights

        trainer.train() # Traineing

        # Flush Memory (Delete any unused data)
        del trainer,model
        gc.collect()
        # GPU Cache
        if self.device == 'cuda':
    ...
    # Simplify jutsu function
    # We only want to have  these 3 jutsu_type	
    ...
    # preparing text data for training machine learning models
    ...
    def load_data(self,data_path):
        ...
        # We only want to have  these 3 jutsu_type	 on whole dataset
    ...
    # If model path doesnot exit then call  the model and then train it
    ...
    def postprocess(self,model_output):
        ...
        for pred in model_output:
            label = max(pred, key=lambda x: x['score'])['label'] # Maximum score
            output.append(label)
        return output


    # Inference function(classify_jutsu)
